...
export const LaserFlow = ({
  ...
  // 自愈：WebGL 上下文丢失且 6s 内未恢复时，自增 gen 重建实例（仅一次，防重建循环闪烁）
  ...
  useEffect(() => {
    ...
    if (!mount) return;    const renderer = new THREE.WebGLRenderer({
      antialias: false,
      alpha: true, // 透明清屏：canvas 自身透明，无需 mix-blend（mix-blend + 父级 mask 在 Chromium 有合成缺陷，会致光柱被裁剪不可见）
      depth: false,
      stencil: false,
    ...
    renderer.setClearColor(0x000000, 0); // 透明清屏：光柱直显于页面背景上
    ...
    const applyBackgroundMode = value => {
      ...
      // 不再使用 mix-blend-mode（Chromium mask+blend 合成缺陷 → 光柱不可见）
    ...
    const setSizeNow = () => {
      // 尺寸兜底：容器量到 0 时用视口 1/4 作为最小画布启动，绝不因 0 尺寸跳过渲染
      const fallbackW = Math.max(1, Math.round((window.innerWidth || 800) / 4));
      const fallbackH = Math.max(1, Math.round((window.innerHeight || 600) / 4));
      const w = mount.clientWidth > 0 ? mount.clientWidth : fallbackW;
      const h = mount.clientHeight > 0 ? mount.clientHeight : fallbackH;
    ...
    ro.observe(mount);
    // resize 仅负责更新画布尺寸（RO 已达该职责）；绝不作为渲染/动画启动开关
    const bootTimer = window.setTimeout(() => {
      // 兜底定时器（250ms）：页面布局不稳定时无条件重设尺寸并首帧启动，不等待任何事件
      lastSizeRef.current = { width: 0, height: 0, dpr: 0 }; // 强制刷新尺寸
      scheduleResize();
    }, 250);

    // 页脚光柱为常驻基础背景：不随视口进出暂停（移除 IO 暂停，仅标签页隐藏时暂停）
    ...
    const onCtxLost = e => {
      ...
      // 6s 后检查：上下文仍未恢复且从未自愈过 → 重建一次（光柱恢复；不循环重建避免闪烁）
    ...
    const animate = () => {
      ...
      if (pausedRef.current) return; // 仅标签页隐藏时暂停；页脚光柱常驻渲染（不再受视口进出影响）
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      canvas.style.filter = lightBackground ? 'invert(1) hue-rotate(180deg)' : 'none';
      // 保持与初始化一致：不再覆盖 mix-blend（避免重置回 screen；screen 与父级 mask 合成缺陷会使光柱不可见）
    }
  }, [
    wispDensity,
    mouseTiltStrength,
