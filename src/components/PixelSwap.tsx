...
const PATTERNS: Record<string, PatternFn | null> = {
  random: () => null,
...
function PixelSwap({
  ...
  useEffect(() => {
    ...
    // 网格尚未完成测量（box 为 0）时下一帧重试，避免空网格导致无动画跳变
    if (!frozenGrid.pixels.length) {
      const retry = window.setTimeout(
        () => setTransition({ to, grid: gridRef.current }),
        16
      );
    ...
    frozenGrid.pixels.forEach((pixel, index) => {
      ...
      // 内容视窗裁剪为像素大小（而非全幅），合成层尺寸骤减，动画更流畅。
      ...
      const half = frozenGrid.size / 2;
      content.style.transformOrigin = `${half}px ${half}px`;

      const clone = source.cloneNode(true) as HTMLElement;
      clone.dataset.visible = "true";
      clone.removeAttribute("aria-hidden");
      clone.style.position = "absolute";
      clone.style.left = `${-pixel.left}px`;
      clone.style.top = `${-pixel.top}px`;
