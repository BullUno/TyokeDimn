...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // 视口裁剪：仅绘制视口附近的行（含 120px 余量），大幅降低主线程负担；
      // 画布随页面滚动，直接依据元素相对视口的矩形计算可见范围
      const rect = canvas.getBoundingClientRect();
      const viewTop = Math.max(0, -rect.top - 120);
      const viewBottom = Math.min(canvas.height, -rect.top + rect.height + 120);
      ...
      if (isHex) {
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (cy + squareSize < viewTop || cy > viewBottom) continue;
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (sy + squareSize < viewTop || sy > viewBottom) continue;
