...
// 光柱（three.js）：站点最底部元素。滚动接近页脚（提前 1600px 预热）时才加载，
// 首屏不再预取 three chunk（约 126 KB gzip）；加载完成后的渲染与渐进效果与原先完全一致。
const LaserFlow = lazy(() => import("./components/LaserFlow"));

/**
 * 滚动接近时再挂载 children（懒加载预热）。
 * 注意：观察节点必须是有 border box 的元素——display:contents 无盒子，
 * IntersectionObserver 不会对其产生相交事件（回归成因），故用 absolute inset-0（与父容器同尺寸）。
 */
function LaserFlowGate({ children }: { children: React.ReactNode }) {
...
export default function App() {
  ...
  // 媒体防护：仅拦截「图片/视频/受保护媒体」的右键菜单（对普通用户增加顺手保存的阻力）。
  // 文字、链接、表单等其余区域右键行为完全不受影响；F12/抓包/截图等高级手段不在此防护范围。
  useEffect(() => {
    const onCtxMenu = (e: MouseEvent) => {
      const t = e.target as HTMLElement | null;
      if (!t) return;
      if (t.closest("img, video, .protect-media")) {
        e.preventDefault();
      }
    };
